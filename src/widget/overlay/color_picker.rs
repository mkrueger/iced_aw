        ...
        + iced_widget::text::Catalog,
{
    ...
    let block1_node = Column::<(), Theme, Renderer>::new()
        .spacing((PADDING.top + PADDING.bottom) / 2.) // Average vertical padding
        .push(
            Row::new()
                .width(Length::Fill)
        ...
        + iced_widget::text::Catalog,
{
    ...
    let mut hex_text_layout = Row::<Message, Theme, Renderer>::new()
        .width(Length::Fill)
        .height(Length::Fixed(
            renderer.default_size().0 + (PADDING.top + PADDING.bottom),
        ))
    ...
    Node::with_children(
        Size::new(
            rgba_bounds.width + (PADDING.left + PADDING.right),
            rgba_bounds.height
                + hex_bounds.height
