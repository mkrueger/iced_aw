    ...
    Theme: Catalog + container::Catalog,
{
    ...
    fn layout(&mut self, tree: &mut Tree, renderer: &Renderer, limits: &Limits) -> Node {
        ...
        let max_width = match self.width {
            Length::Shrink => self
                ...
                .map(|(id, val)| {
                    ...
                    (state.values[id].min_bounds().width + (self.padding.left + self.padding.right)).round() as u32
                })
                .max()
                .unwrap_or(100),
        ...
        let limits = limits.max_width(max_width as f32 + (self.padding.left + self.padding.right));
